...
import { Clock, Zap, MapPin, Target, Activity, Download, Edit, Trash2, Calendar as CalendarIcon, FileText } from 'lucide-react';
...
const DailySummaryModal: React.FC<DailySummaryModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'plans' | 'note'>('summary');
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'note':
        return (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white">Nota del Día</h3>
            {dayData.note ? (
              <div className="p-4 bg-primary-700/30 rounded-lg border border-primary-500/30">
                <div className="flex items-start space-x-3">
                  <FileText className="w-5 h-5 text-primary-300 mt-1" />
                  <div className="text-white">{dayData.note}</div>
                </div>
              </div>
            ) : (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-primary-800 border-primary-500/20 text-white max-h-[90vh] overflow-y-auto">
        ...
        {/* Tab Navigation */}
        <div className="flex space-x-1 mb-4 bg-primary-700/40 p-1 rounded-lg">
          <Button
            onClick={() => setActiveTab('summary')}
            variant={activeTab === 'summary' ? 'default' : 'ghost'}
            size="sm"
            className={`flex-1 ${activeTab === 'summary' ? 'bg-primary-600 text-white' : 'text-primary-300 hover:bg-primary-600/50'}`}
          >
          ...
          <Button
            ...
            size="sm"
            className={`flex-1 ${activeTab === 'plans' ? 'bg-primary-600 text-white' : 'text-primary-300 hover:bg-primary-600/50'}`}
          >
          ...
          <Button
            ...
            size="sm"
            className={`flex-1 ${activeTab === 'note' ? 'bg-primary-600 text-white' : 'text-primary-300 hover:bg-primary-600/50'}`}
          >
